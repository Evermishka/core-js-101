...
function encodeToRot13(str) {
  ...
  function changeIndex(i) {
    const j = (i > 12) ? (i - 13) : (i + 13);
    return j;
  }
  for (let i = 0; i < str.length; i += 1) {
    if ((uppercaseSymbols.indexOf(str[i]) === -1) && (lowercaseSymbols.indexOf(str[i]) === -1)) {
      newStr += str[i];
    } else {
      let index;
      if (str[i].toUpperCase() === str[i]) {
        index = changeIndex(uppercaseSymbols.indexOf(str[i]));
        newStr += uppercaseSymbols[index];
      } else {
        index = changeIndex(lowercaseSymbols.indexOf(str[i]));
        newStr += lowercaseSymbols[index];
      }
    }
  }
  return newStr;
